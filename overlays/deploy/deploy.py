...
def route(frm: str, to: str) -> str:
    special = {
        ("mac-akuma-aarch64", "mac-linux-aarch64"): "http://10.0.2.2:9944",
        ("mac-linux-aarch64", "mac-akuma-aarch64"): "http://10.0.2.15:9944",
    }
    if (frm, to) in special:
        return special[(frm, to)]
    if to == "dumpster-akuma-amd64":
        return "http://192.168.1.123:9944"
    if to == "ryzen-linux-amd64":
        return "http://192.168.1.126:9944"
    if to == "mac-linux-aarch64":
        return f"http://{MAC_LAN}:9944"
    if to == "mac-akuma-aarch64":
        return f"http://{MAC_LAN}:9945"
...
def cmd_ids() -> None:
    ...
    for name in ("ryzen-akuma-amd64", "mac-akuma-aarch64"):
    ...
    members = [root_acct] + [acct for _, acct in accounts]
    roster = ["root=pub:" + root_acct] + [f"{name}=pub:{acct}" for name, acct in accounts]
    leader = accounts[0][1]

    lines = [
        f"# Genesis for the 5-agent mesh — generated by deploy.py ids, {datetime.date.today().isoformat()}.",
        "# Public keys only. Every node must agree on all of this: it is genesis.",
        f'MIOT_ROOT_PUBKEY="{root_line}"',
        f"MIOT_LEADER={leader}",
        f"MIOT_MEMBERS={','.join(members)}",
...
def env_for(name: str) -> str:
    ...
    a = agent(name)
    peers = ",".join(route(name, p) for p in LIVE if p != name)
    mesh = _read_mesh_env()
    ...
    lines += [
        ...
        f'MIOT_MEMBERS={mesh["MIOT_MEMBERS"]}',
