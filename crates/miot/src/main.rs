...
/// Run as a chain node — `miot node`. Everything above this point in the
/// file is this function's supporting cast (`Node`, `Role`, the axum
/// handlers, the sync/reconcile loops). Dispatched from `main`, at the very
/// bottom of the file, alongside the demo/`--rpc`/`--live`/`--chat` client
/// modes this binary also has.
async fn run_node() {
