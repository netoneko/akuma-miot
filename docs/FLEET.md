...
|---|---|---|---|---|
| meow | `dumpster-akuma-amd64` | akuma (metal) | `glm-5.3-flash`, reasoning `low`, 1M window (config) — `glm-5.3` until 2026-09-25; staged without a restart, live after its own next reboot | z.ai coding plan (`--glm`); pushes to `akuma-litter` (token) |
| tama | `ryzen-linux-amd64` | ryzen | `glm-5.3-flash`, reasoning `low`, 1M window (config) — `glm-5.3` until 2026-09-25; redeployed with it 2026-09-26 | z.ai coding plan (`--glm`) — was Qwen3-4B on ryzen's llama-server until 2026-09-25; pushes to `akuma-litter` |
